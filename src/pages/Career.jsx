...
import { departmentCareers, engineeringDepartments } from '../data/mockData';
import {
  Compass,
  Bot,
  Code,
...
export default function Career() {
  const { setCurrentPage, user, changeDepartment } = useLearning();

  const activeCareers = departmentCareers[user.department] || departmentCareers.ai_ds;
  const [selectedRole, setSelectedRole] = useState(activeCareers[0]);

  // Sync selected role when department changes
  useEffect(() => {
    const newCareers = departmentCareers[user.department] || departmentCareers.ai_ds;
    setSelectedRole(newCareers[0]);
  }, [user.department]);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="rounded-3xl bg-white p-6 sm:p-7 border border-slate-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500">
            Targeted job role matching and industry readiness for <strong>{activeDeptInfo.name}</strong>.
          </p>
        </div>
      ...
      <div className="rounded-2xl bg-white p-3 border border-slate-200/70 shadow-sm flex items-center gap-2 overflow-x-auto">
      ...
      {selectedRole && (
        <div className="rounded-3xl bg-white p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-4">
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-xl font-black text-slate-900">{selectedRole.title} Readiness</h2>
                <span className="rounded-full bg-indigo-50 px-2.5 py-0.5 text-xs font-bold text-indigo-700">
                  {selectedRole.matchPercentage}% Fit
                </span>
              </div>
              <p className="text-xs text-slate-500 mt-1">
                Bridging academic coursework with top industry job criteria.
              </p>
            </div>
            ...
            >
              <span>Bridge Gaps in Learning Path</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="rounded-2xl bg-rose-50/70 p-5 border border-rose-100 space-y-3">
              <div className="flex items-center gap-2 text-rose-900 font-bold text-sm">
                ...
                <span>Industry Gap Checklist ({selectedRole.missingSkills.length})</span>
              </div>
              <div className="space-y-2">
